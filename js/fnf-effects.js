...
class FoodNForceEffects {
    ...
    setupScrollProgress() {
        if (document.querySelector('.fnf-scroll-progress')) return;
    ...
    setupCardTilt(card) {
        ...
        this.eventListeners.push(
            { element: card, event: 'mousemove', handler: mousemoveHandler },
            { element: card, event: 'mouseleave', handler: mouseleaveHandler }
        );
    }
    ...
    animateElement(element) {
        if (element.classList.contains('fnf-animated')) return;
    ...
    animateStats(statContainer) {
        if (statContainer.classList.contains('fnf-counted')) return;
    ...
    setupMagneticEffect(button) {
        if (this.prefersReducedMotion || !button.classList.contains('slds-button_brand')) return;
    ...
    createHeroParticles() {
        const heroSection = document.querySelector('.hero-section');
        if (!heroSection || this.prefersReducedMotion) return;
        
        // Check if particles already exist
        if (heroSection.querySelector('.fnf-particle-container')) return;
    ...
    createNewsletterModal() {
        if (document.querySelector('.fnf-newsletter-overlay')) return;
    ...
    createIridescentBackground() {
        if (document.querySelector('.fnf-iridescent-background')) return;
